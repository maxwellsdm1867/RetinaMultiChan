function  sort_extract(merge_file_dir,name_seq,trial_time,rslt_dir,mcd_dir)
...
% input
if size(name_seq,2)~=length(trial_time)
    display('check the seq name and duration')
...
for i = 1:size(name_seq,2)
    ...
    extracted.analog = a_data(2,1:round(size(a_data,2)/5));
    cd(rslt_dir)
    save(name_seq{i},'extracted')
